...
public class PaginaDefinicionMarcas {
	...
	private static final Log logger = LogFactory
			.getLog(PaginaDefinicionMarcas.class);

	private int numPagina; // página sobre la cuál versa la información
	private Hashtable<String, Campo> campos = new Hashtable<String, Campo>(); // Hastable
																				// para
																				// almacenar
																				// los
																				// campos
																				...
																				// de
																				// marcas
	private Vector<String> marcas = new Vector<String>(); // vector para buscar
															// marcas, aquí
															// están almacenadas
															// las keys
	...
	/**
	 * Método que lee las marcas de un objeto BufferedReader y las almacena en
	 * un objeto tipo Campo
	 * 
	 * @param in
	 * @throws IOException
	 */
	public void leerMarcas(BufferedReader in) throws IOException {
		...
		in.mark(20); // marcamos para recordar la posición anterior donde
						// termino la lectura de in
		while ((line = in.readLine()) != null && !line.equals("")) {
			if (line.startsWith("[Page")) // etiqueta de principio de página
			{
				if (logger.isDebugEnabled()) {
					logger
							.debug("leerMarcas(BufferedReader) - Página siguiente"); //$NON-NLS-1$
				}
				in.reset();
				return;
			} else // lectura de campos de una línea
			{
				...
				marcas.add(campo.getNombre()); // almacenamos en el array
												// marcas[] la clave
			}
			in.mark(20);
		}
	...
	/**
	 * Método que devuelve el vector marcas, que contiene las claves de los
	 * campos
	 * 
	 * @return marcas
	 */
