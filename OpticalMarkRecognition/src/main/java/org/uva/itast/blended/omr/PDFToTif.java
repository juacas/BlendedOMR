...
public class PDFToTif
{
	...
	public void reescalar(String filename)
	{
		...
		try
		{
	        ...
	        //leemos la imagen como un archivo y la almacenamos en un objeto imagen
	        ...
	        //configuramos la imagen con las medidas especificas y en escala de grises
	        imagenSalida = new BufferedImage(resizeWidth, resizeHeight, BufferedImage.TYPE_BYTE_GRAY );
	        //creamos un objeto gráfico en dos dimensiones
	        imagenSalida.createGraphics();
			imagenSalida.createGraphics().drawImage(imagenObjeto, 0, 0, resizeWidth, resizeHeight, null);
			//se guarda en el formato tif
			salvarImagen(imagenSalida,filename + ".tif");
	        System.out.println("Finalizando reescalado de imagen");
			/*
			//Prueba para visualizar la imágen en una ventana externa
	        JFrame frame = new JFrame();
	        JLabel label = new JLabel(new ImageIcon(imagenSalida));
    ...
    public static void salvarImagen(Image imagen, String filename)
    {
       ...
       {           
    	   ...
    	   //se elige el formato en el cual se guardará la imagen y se escribe en un fichero
	...
	private static BufferedImage leerImagenPDF(PDFPage page)
    {
	        ...
	        //configuramos la imagen con las medidas especificas y en escala de grises
	        img_pdf = new BufferedImage(resizeWidth, resizeHeight, BufferedImage.TYPE_BYTE_GRAY );
	        //creamos un objeto gráfico en dos dimensiones
			Graphics2D g2 = img_pdf.createGraphics();
			g2.setRenderingHint(RenderingHints.KEY_ANTIALIASING, RenderingHints.VALUE_ANTIALIAS_ON);

			//renderizamos la imgen
			PDFRenderer renderer = new PDFRenderer(page, g2, new Rectangle(0, 0, 1700, 2339), null, Color.RED);
			page.waitForFinish();
			renderer.run();
			//por último dibujamos la imagen
	        img_pdf.createGraphics().drawImage(img_pdf, 0, 0, resizeWidth, resizeHeight, null);
		}
		catch(Exception e)
	       {
	           e.printStackTrace();
