...
public final class BarcodeManipulation {
	...
	private static final Log logger = LogFactory
			.getLog(BarcodeManipulation.class);
	...
	/**
	 * Método que lee el valor de un código de barras contenido en un objeto
	...
	public Result leerBarcode(Campo campo) throws ReaderException {
		...
		try {
			...
			// for report
			subimage = subimage.getSubimage(area.x - expandedArea.x, area.y
					- expandedArea.y, area.width, area.height);
		}
	...
	private Result scanAreaForBarcode(Rectangle rect) throws ReaderException {
		...
		// BufferedImageMonochromeBitmapSource seems to work bad with
		// TYPE_BYTERGB

		BufferedImage barcodeArea = pageImage.getImagen().getSubimage(rect.x,
				rect.y, rect.width, rect.height); // se coge la subimagen,
													// x,y,w,h (en píxeles)
		
		/*XXX borrar, prueba para ver que no reconoce el barcode atravesado por líneas
		  File rasterImageFile = new File("C:\\Documents and Settings\\Administrador\\Escritorio\\hola.png");
		  try {
			ImageIO.write(barcodeArea,"png", rasterImageFile);
		...
		if (barcodeArea == null) {
			logger
					.error(
							"leerBarcode(Campo) - " + pageImage.getImagen().toString() + ": No es posible cargar la imagen", null); //$NON-NLS-1$ //$NON-NLS-2$
			// TODO: Lanzar Excepcion
		}

		subimage = new BufferedImage(rect.width, rect.height,
		...
		try {
			...
			if (medianfilter == true) {
				...
				g2.drawString("MEDIAN", medianed.getWidth() / 2 - 20, medianed
						.getHeight() / 2);
	...
	public String getParsedCode(Result result) {
		...
		// System.out.println(imagen.toString() + " (format: " +
	...
	private Rectangle getRectArea(double[] coords) {
		int x = this.pageImage.toPixelsX(coords[0]); // concentriccirclewidth//posición
														// de la x
	...
	/**
	 * Aplica un filtro para reconstruir imagenes de mala calidad, a través del
	 * valor de los píxeles vecinos
	 * 
	 * @param subimage
	 */
	private static BufferedImage medianFilter(BufferedImage subimage) {
		...
		BufferedImage result = filter.createCompatibleDestImage(subimage,
	...
	public void markBarcode(Campo campo) {
		...
		g.drawRoundRect(expandedRect.x, expandedRect.y, expandedRect.width,
				expandedRect.height, 3, 3);

		if (lastResult != null)
			g.drawString(lastResult.getBarcodeFormat().toString() + "="
					+ getParsedCode(lastResult), rect.x, rect.y);
	...
	private Rectangle getExpandedArea(double coords[]) {
		...
		Rectangle expandedRect = new Rectangle((int) (rect.x - rect.width
				* (BARCODE_AREA_PERCENT) / 2), (int) (rect.y - rect.height
				* (BARCODE_AREA_PERCENT) / 2),
