...
public abstract class AbstractAlignMarkDetector implements AlignMarkDetector
{
	...
	public AbstractAlignMarkDetector()
	{
		super();
	}

	public AbstractAlignMarkDetector(OMRTemplate template)
	{
	...
	/* (non-Javadoc)
	 * @see org.uva.itast.blended.omr.pages.AlignMarkDetector#setBufferWidth(int)
	 */
	...
	/* (non-Javadoc)
	 * @see org.uva.itast.blended.omr.pages.AlignMarkDetector#getAlignmentSlope()
	 */
	...
	/* (non-Javadoc)
	 * @see org.uva.itast.blended.omr.pages.AlignMarkDetector#setAlignmentSlope(double)
	 */
	...
	/* (non-Javadoc)
	 * @see org.uva.itast.blended.omr.pages.AlignMarkDetector#align(org.uva.itast.blended.omr.pages.PageImage)
	 */
	public Point2D[] align(PageImage pageImage)
	{
		...
		//Rotaciï¿½n
	if (PageImage.logger.isDebugEnabled())
	{
		PageImage.logger.debug("align(OMRTemplate, PageImage) - ï¿½ngulo de rotaciï¿½n: " + angulo_rotacion*180/Math.PI+" degrees."); //$NON-NLS-1$
	}
	...
	if (PageImage.logger.isDebugEnabled())
	{
		PageImage.logger.debug("align(OMRTemplate, PageImage) - Traslado de x: " + punto_translado.getX()); //$NON-NLS-1$
		PageImage.logger.debug("align(OMRTemplate, PageImage) - Traslado de y: " + punto_translado.getY()); //$NON-NLS-1$
	}
	...
	if (PageImage.logger.isDebugEnabled())
	{
		PageImage.logger.debug("align(OMRTemplate, PageImage) - Escala de x: " + escala_x); //$NON-NLS-1$
		PageImage.logger.debug("align(OMRTemplate, PageImage) - Escala de y: " + escala_y); //$NON-NLS-1$
	}
	//TODO check this way of applying the transformation. Shoul reuse and modify original transform.
...
/* (non-Javadoc)
 * @see org.uva.itast.blended.omr.pages.AlignMarkDetector#searchAlignMarks(org.uva.itast.blended.omr.Field, org.uva.itast.blended.omr.pages.PageImage)
 */
public Point2D[] searchAlignMarks(Field campo, PageImage pageImage)
{
	...
	Point2D dtopleft = pointPosition(pageImage, etopleft);			//detected top left point
	Point2D dtopright = pointPosition(pageImage, etopright);		//detected top right point
	Point2D dbottomleft = pointPosition(pageImage, ebottomleft);		//detected bottom left point
	Point2D dbottomright = pointPosition(pageImage, ebottomright);		//detected bottom right point
	...
	double pend_align_1 = (dtopright.getY()-dtopleft.getY())/(dtopright.getX()-dtopleft.getX());
	double pend_align_2 = (dbottomright.getY()-dbottomleft.getY())/(dbottomright.getX()-dbottomleft.getX());;
...
protected void logImage(BufferedImage subimage)
{
	if (logger.isDebugEnabled())
	{
		try
		{
			File rasterImageFile=File.createTempFile("debugCorners", ".png");
